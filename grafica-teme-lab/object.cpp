...
void Object::transform(maybe<f3> location, maybe<f3> rotation, maybe<f3> scale) const {
	this->use();
	...
	m4 new_matrix = this->original_matrix
		* glm::translate(m4(1.0f), location.value())
...
void Object::transform_mutable(maybe<f3> location, maybe<f3> rotation, maybe<f3> scale) {
	this->use();
...
void Object::transform_delta(maybe<f3> delta_location, maybe<f3> delta_rotation, maybe<f3> delta_scale) const {
	this->use();
	...
	f3 loc = this->location + delta_location.value();
	f3 rot = this->rotation + delta_rotation.value();
	f3 scl = this->scale * delta_scale.value();
...
void Object::transform_delta_mutable(maybe<f3> delta_location, maybe<f3> delta_rotation, maybe<f3> delta_scale) {
	this->use();
...
f3 Object::get_real_location() const {
	f3 loc = m4_make(this->get_uniform_value<f32>("myMatrix").data())[3];
	loc = f3(
		vmap(loc.x, -1.0f, 1.0f, app::camera.xmin,  app::camera.xmax),
		vmap(loc.y, -1.0f, 1.0f, app::camera.ymin,  app::camera.ymax),
		vmap(loc.z, -1.0f, 1.0f, app::camera.znear, app::camera.zfar)
	);
	return loc;
}
